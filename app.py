...
st.title("💧 Sistema de Auditoría y Gestión de Tiempos — Dinagua")
st.markdown("""
Esta aplicación interactiva permite explorar el comportamiento histórico de los tiempos de resolución 
de expedientes hídricos, filtrando por rangos temporales y analizando el impacto de las variables clave.
""")
...
# Filtro 2: Selector complementario por Departamento (Opcional interactivo para enriquecer)
departamentos_disponibles = [col.replace("Departamento_", "") for col in df_modelo.columns if "Departamento_" in col]
depto_seleccionado = st.sidebar.selectbox("Filtrar por foco de Departamento (Visual):",
                                          ["TODOS"] + departamentos_disponibles)

# APLICACIÓN DE LOS FILTROS AL DATAFRAME ORIGINAR
# Filtrado por rango del slider
...
if not df_filtrado.empty:
    ...
    st.dataframe(df_filtrado[['Dias_Demora', 'Es_Riego']].describe().T)
else:
...
with col_graf1:
    st.markdown("### 📈 Distribución del Target (`Dias_Demora`)")
    if not df_filtrado.empty:
        fig_hist, ax_hist = plt.subplots(figsize=(6, 4))
        sns.histplot(df_filtrado['Dias_Demora'], kde=True, color="#2c3e50", ax=ax_hist, bins=25)
        ax_hist.set_title("Histograma de Frecuencias de Demora")
